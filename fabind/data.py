...
class FABindDataSet(Dataset):
    def __init__(
        ...
        compound_coords_init_mode=None,
        seed=42,
        pre=None,
        transform=None,
        pre_transform=None,
        pre_filter=None,
        noise_for_predicted_pocket=5.0,
        ...
        if use_esm2_feat:
            self.protein_esm2_feat = lmdb.open(
    ...
    def get(self, idx):
        ...
        use_whole_protein   = (
            line["use_whole_protein"]
            if "use_whole_protein" in line.index
            else self.use_whole_protein
        )
        ...
        if self.proteinMode == 0:
            with self.protein_dict.begin() as txn:
                protein_node_xyz, protein_seq = pickle.loads(
                    txn.get(protein_name.encode())
                )
        ...
        with self.compound_dict.begin() as txn:
            (
                ...
                LAS_edge_index,
            ) = pickle.loads(txn.get(name.encode()))
        
        if self.proteinMode == 0:
            data, input_node_list, keepNode = construct_data_from_graph_gvp_mean(
                self.args,
                protein_node_xyz,
                protein_seq,
                coords,
                ...
                pocket_idx_no_noise=self.pocket_idx_no_noise,
...
def get_data(
    ...
    if args.data == "0":
        ...
        new_dataset = FABindDataSet(
            ...
            use_esm2_feat=args.use_esm2_feat,
            seed=args.seed,
            pre=pre,
            args=args,
        )
        # ['pdbbind2020/dataset/processed/data.pt', 'pdbbind2020/dataset/processed/protein_1d_3d.lmdb',
        # 'pdbbind2020/dataset/processed/compound_LAS_edge_index.lmdb',
        # 'pdbbind2020/dataset/processed/compound_rdkit_coords.pt', 'pdbbind2020/dataset/processed/esm2_t33_650M_UR50D.lmdb']

        # print(new_dataset.data) # nguyên cái bảng data
        ...
        train_tmp = new_dataset.data.query(
            "c_length < 100 and native_num_contact > 5 and group =='train' and use_compound_com"
        ).reset_index(drop=True)
        valid_test_tmp = new_dataset.data.query(
            "(group == 'valid' or group == 'test') and use_compound_com"
        ).reset_index(drop=True)
        new_dataset.data = pd.concat([train_tmp, valid_test_tmp], axis=0).reset_index(
            drop=True
        )
        d = new_dataset.data
