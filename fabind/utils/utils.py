...
def read_pdbbind_data(fileName):
    ...
    info = pd.DataFrame(
        info, columns=["pdb", "resolution", "year", "affinity", "raw", "ligand"]
    )
...
def get_keepNode(
    ...
    # print(protein_node_xyz.shape)
    # print(com, n_node, pocket_radius, use_whole_protein,
...
def get_keepNode_tensor(
    protein_node_xyz, pocket_radius, add_noise_to_com, chosen_pocket_com
):
    if add_noise_to_com:
    ...
    dis = compute_dis_between_two_vector_tensor(
        protein_node_xyz, chosen_pocket_com.unsqueeze(0)
    )
...
def construct_data_from_graph_gvp_mean(
    ...
    if args.local_eval:
        if group == "test":
            ...
            pre = args.data_path
            mol, _ = read_mol(
                f"{pre}/renumber_atom_index_same_as_smiles/{pdb_id}.sdf", None
            )
    ...
    if args.train_ligand_torsion_noise and group == "train":
        ...
        try:
            mol = Chem.MolFromMolFile(
                f"{pre}/renumber_atom_index_same_as_smiles/{pdb_id}.sdf", sanitize=False
            )
            try:
                Chem.SanitizeMol(mol)
            except:
            ...
            raise ValueError(
                f"cannot find {pdb_id}.sdf in {pre}/renumber_atom_index_same_as_smiles/"
            )
        rotable_bonds = get_torsions(mol)
...
@torch.no_grad()
def evaluate_mean_pocket_cls_coord_multi_task(
    accelerator,
    args,
    data_loader,
    model,
    ...
    for data in data_iter:
        ...
        (
            ...
            pocket_coord_pred_direct,
            dis_map,
            keepNode_less_5,
        ) = model(data, stage=stage)
        # y = data.y
        ...
        if pred_dis:
            ...
            contact_distill_loss = (
                args.pair_distance_distill_loss_weight
                * criterion(y_pred_by_coord, y_pred)
                if len(y_pred) > 0
                else torch.tensor([0])
            )
        else:
            contact_loss = (
                criterion(y_pred, dis_map) if len(dis_map) > 0 else torch.tensor([0])
            )
            y_pred = y_pred.sigmoid()

        pocket_cls_loss = args.pocket_cls_loss_weight * pocket_cls_criterion(
            pocket_cls_pred, pocket_cls.float()
        )
        pocket_coord_direct_loss = (
            args.pocket_distance_loss_weight
            * pocket_coord_criterion(pocket_coord_pred_direct, data.coords_center)
        )

        com_coord_loss = args.coord_loss_weight * com_coord_criterion(
        ...
        pocket_coord_direct_batch_loss += (
            len(pocket_coord_pred_direct) * pocket_coord_direct_loss.item()
        )
        ...
        for i, j in enumerate(batch_len):
            ...
            if pred_index_bool.sum() != 0:
                pred_pocket_center = (
                    p_coords_batched_whole.detach()[i][:j][pred_index_bool]
                    .mean(dim=0)
                    .unsqueeze(0)
                )
                ...
                pred_index_one_hot = gumbel_softmax_no_random(
                    pred_index_log_prob, tau=args.gs_tau, hard=False
                )
                pred_index_one_hot_true = pred_index_one_hot[:, 1].unsqueeze(-1)
                pred_pocket_center_gumbel = (
                    pred_index_one_hot_true * p_coords_batched_whole[i][:j]
                )
                pred_pocket_center_gumbel_mean = pred_pocket_center_gumbel.sum(
                    dim=0
                ) / pred_index_one_hot_true.sum(dim=0)
                pocket_coord_pred_list.append(
                    pred_pocket_center_gumbel_mean.unsqueeze(0).detach()
                )
                pocket_coord_list.append(data.coords_center[i].unsqueeze(0))
    ...
    pocket_cls_accuracy = (pocket_cls_pred == pocket_cls).sum().item() / len(
        pocket_cls_pred
    )
    ...
    metrics.update(
        {"pocket_cls_bce_loss": pocket_cls_batch_loss / len(pocket_cls_pred_list)}
    )
    metrics.update(
        {
            "pocket_coord_mse_loss": pocket_coord_direct_batch_loss
            / len(pocket_coord_pred_direct)
        }
    )
...
@torch.no_grad()
def evaluate_mean_pocket_cls_coord_pocket_pred(
    args,
    data_loader,
    model,
    com_coord_criterion,
    ...
    for data in tqdm(data_loader, mininterval=args.tqdm_interval):
        ...
        for i, j in enumerate(batch_len):
            ...
            if pred_index_bool.sum() != 0:
                ...
                pocket_coord_pred_list.append(
                    pred_pocket_center_gumbel_mean.detach().cpu().numpy()
                )
                pocket_coord_list.append(data.coords_center[i].unsqueeze(0))
