...
user_name = 'phucpht'
# ====== Global variables ======
path_pdbs =  sorted(glob(f"/cm/archive/{user_name}/Drug_research/Blind_pocket/data/{folder}/pdb/*.pdb"))
# path_generate_ligands = sorted(glob(f"/cm/shared/{user_name}/Drug_research/Blind_pocket/{folder}/docking_gen_ligands/*csv"))
# path_pdbqts = f"/cm/archive/{user_name}/Drug_research/Blind_pocket/{folder}/pdb"
name_dataset = "drugbank"
dataset_path = f"datasets/{name_dataset}.csv"
device = torch.device("cuda:0")
idx = int(sys.argv[1])
Seed_everything(seed=42)
pdb_path = path_pdbs[idx]
protein_name = os.path.split(pdb_path)[1].replace(".pdb", "")

if not os.path.exists(f'data/{folder}/remove_water/'):
    os.makedirs(f'data/{folder}/remove_water/')

remove_water(pdb_path, f'data/{folder}/remove_water/{protein_name}.pdb')
receptor_path = f'data/{folder}/remove_water/{protein_name}.pdbqt'
convert_pdb_to_pdbqt(f'data/{folder}/remove_water/{protein_name}.pdb',receptor_path)
protein_remove_water = f'data/{folder}/remove_water/{protein_name}.pdb'
...
def pocket_position_to_coordinate(path_pocket):
    ...
    if not os.path.exists(f'data/{folder}/generation/{protein_name}'):
        os.makedirs(f'data/{folder}/generation/{protein_name}')

    pocket_path = f'data/{folder}/generation/{protein_name}/{pocket_name}.sdf' 
    if not os.path.exists(pocket_path):
        cmd = f"python diffusion_generate/generate_ligands.py diffusion_generate/checkpoints/crossdocked_fullatom_cond.ckpt --pdbfile {pdb_path} --outfile data/{folder}/generation/{protein_name}/{pocket_name}.sdf --resi_list {residue_lst}  --n_samples 10 --device {device}"
        os.system(cmd)
...
for path_pocket in glob(f'data/{folder}/remove_water/{protein_name}_out/pockets/*.pdb'):
    ...
    for i in range(len(smile_list)):
        ...
        df.to_csv(f'data/{folder}/generation/{protein_name}/generation.csv', index = False)


def docking(generation_path, receptor_path):
    if os.path.exists(f'data/{folder}/generation_docking/generation_docking.csv'):
        return f'data/{folder}/generation_docking/generation_docking.csv'
    ...
    for i in range(len(df)):
        ...
        if not os.path.exists(f'data/{folder}/generation_docking/'):
            os.makedirs(f'data/{folder}/generation_docking/')
        ba_generation = func(f"{protein_name}_{row['key_idx']}.pdbqt", f"data/{folder}/generation_docking/{row['key_idx']}.pdbqt", n_poses = 5)
        df.loc[i, 'ba'] = ba_generation
        os.remove(f"{protein_name}_{row['key_idx']}.pdbqt")
        df.to_csv(f'data/{folder}/generation_docking/generation_docking.csv', index = False)
    return f'data/{folder}/generation_docking/generation_docking.csv'
 
path_generate_docking = docking(f'data/{folder}/generation/{protein_name}/generation.csv', receptor_path)

def preprocessed_data(path_generate_ligand):
    out_folder = f"data/{folder}/preprocessed_data/"
...
preprocessed_data(path_generate_docking)
exit()
def binding_data_creation(preprocessed_data_path, path_pdbqt=path_pdbqts):
...
def binding_fine_tune(out_ligand):
    ...
    protein_path = os.path.join(path_pdbqts, f"{protein_name}.pdbqt")
...
# print(path_generate_ligands[idx])
preprocessed_data_path = preprocessed_data(path_generate_ligands[idx])
# preprocessed_data_path = "preprocessed_data/preprocessed_1a42_drugbank_data.csv"
protein_name = os.path.split(preprocessed_data_path)[1].split('_')[1]
dataset_name = name_dataset
out_file, out_ligand   = binding_data_creation(preprocessed_data_path)
# out_file = f"/cm/shared/{user_name}/Molecule_Generation/Drug_repurposing/DrugBank/binding_data/binding_data_1a42_drugbank.csv"
# out_ligand = f"/cm/shared/{user_name}/Molecule_Generation/Drug_repurposing/DrugBank/docking_output/1a42_drugbank_binding_ligands_stage_1"
optimal_weight_path = binding_fine_tune(out_ligand)
# optimal_weight_path = "/cm/shared/{user_name}/Molecule_Generation/Drug_repurposing/DrugBank/fabind/ckpt/best_model.bin"
