...
def tanimoto_score(path_protein, path_dataset):
    ...
    name_protein = os.path.split(path_protein)[1]
    name_dataset = os.path.split(path_dataset)[1]
    output = []
    ...
    out_folder = "/cm/shared/phucpht/Molecule_Generation/Drug_repurposing/DrugBank/similarity_score"
    if not os.path.exists(out_folder):
        os.mkdir(out_folder)
...
def gat_embed_data(encoder_model, dataloader, device):
    encoder_model.eval()
    embed_lst = []
    for data in dataloader:
        data = data.to(device)
...
def create_embedding(name_file="6dql", device = "cuda:4"):
...
def extract_torchdrug_feature_from_mol_conformer(ligand_path):
    pdb = ligand_path.replace('.pdbqt', '.pdb')
    cmd = f"obabel {ligand_path} -O {pdb}"
    os.system(cmd)
    mol = Chem.MolFromPDBFile(pdb, removeHs=True)

    import pdbreader
    df = pdbreader.read_pdb(pdb)['ATOM']
    df = df[df['model_id'] == 0]
    coordinates = df[df['element'] != 'H'][['x', 'y','z']]
...
def extract_torchdrug_database(smiles):
    ...
    # AllChem.EmbedMultipleConfs(rdkit_mol, 1,
    #                            maxAttempts = 1000) #can allow user to change numConfs
...
def create_inference_data(folder_path, protein_path):
    if os.path.exists(os.path.join(folder_path, 'inference_data', f"./inference_data.pt")):
        return
    else:
        os.makedirs(os.path.join(folder_path, 'inference_data'))
    protein_node_xyz, protein_esm_feature = esm2_new(protein_path)
    protein_name = os.path.split(protein_path)[1].replace('.pdbqt', '')
    database_df    = pd.read_csv(os.path.join(folder_path, protein_name + '.csv'))
    index_data     = 0
    inference_data = []
    for i in tqdm(range(len(database_df))):
        try:
            smiles = database_df['smiles'][i]
            (
                mol,
                rdkit_coords,
                compound_node_features,
            ...
            segment = torch.zeros(n_protein_whole + n_compound + 2).int()
            segment[n_compound+1:] = 1 # compound: 0, protein: 1
            data['complex_whole_protein'].segment = segment # protein or ligand
            mask = torch.zeros(n_protein_whole + n_compound + 2).int()
            mask[:n_compound+2] = 1 # glb_p can be updated
            data['complex_whole_protein'].mask = mask.bool()
            is_global = torch.zeros(n_protein_whole + n_compound + 2).int()
            is_global[0] = 1
            is_global[n_compound+1] = 1
            data['complex_whole_protein'].is_global = is_global.bool()
    ...
    torch.save(inference_data, os.path.join(folder_path, 'inference_data', f"./inference_data.pt"))
    del inference_data

def get_inference_data(folder_path):
    path = os.path.join(folder_path, f'inference_data/inference_data.pt')
    if not os.path.exists(path):
        create_inference_data(folder_path)
    return torch.load(path)


def get_simulate_data(args, protein_path , group = "train", data = "binding", i = 1):
    lst_data = []
    protein_name = os.path.split(protein_path)[1].replace('.pdbqt', '')
    if data == 'generation':
        df_generation           = pd.read_csv(glob(
        f'/cm/archive/phucpht/Drug_research/Blind_pocket/datasets/dude_process/{protein_name}/generation_docking/*.csv'
        )[0])
        if ((i-1)*50 > len(sorted(glob(
        f'/cm/archive/phucpht/Drug_research/Blind_pocket/datasets/dude_process/{protein_name}/generation_docking/*.pdbqt'
        )))):
            return lst_data
        compound_lst_generation = sorted(glob(
        f'/cm/archive/phucpht/Drug_research/Blind_pocket/datasets/dude_process/{protein_name}/generation_docking/*.pdbqt'
        ))[((i-1)*50):(i*50)]
        protein_node_xyz, protein_esm2_feat = esm2_new(protein_path)
        for idx, compound_path in enumerate(compound_lst_generation):
            try:
                ...
                pdb_file = compound_path.replace('.pdbqt', '.pdb')
                import pdbreader
                df = pdbreader.read_pdb(pdb_file)['ATOM']
                df = df[df['model_id'] == 1]
                for attribute in ['x','y','z']:
                    df[attribute] -= np.mean(df[attribute])

                coordinates = df[df['element'] != 'H'][['x', 'y','z']]
                rdkit_coords = np.array(coordinates)
        ...
        protein_node_xyz, protein_esm2_feat = esm2_new(protein_path)
        df_binding           = pd.read_csv(glob(
        f'/cm/archive/phucpht/Drug_research/Blind_pocket/datasets/dude_process/{protein_name}/binding_data/*.csv'
        )[0])
        compound_lst_binding = glob(
        f'/cm/archive/phucpht/Drug_research/Blind_pocket/datasets/dude_process/{protein_name}/binding_data/*.pdbqt'
        ) 
        for idx, compound_path in enumerate(compound_lst_binding):
            try:
                compound_name = os.path.split(compound_path)[1].split(".")[0]
                (
                    coords,
                ...
                smiles = df_binding["smiles"][int(compound_name)]
